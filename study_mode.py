...
class StudyModeWindow(tk.Toplevel):
    ...
    def __init__(self, parent, reglament, on_back=None, on_close=None):
        ...
        self.process_indices = {}  # Map listbox index to process index
    ...
    def load_reglament_structure(self):
        ...
        self.process_listbox.itemconfig(current_index, {'fg': '#FF5722'})
        self.process_indices[current_index] = ("global", 0)
        current_index += 1

        # Разделитель
        self.process_listbox.insert(tk.END, "-" * 30)
        self.process_listbox.itemconfig(current_index, {'fg': '#ccc'})
        current_index += 1
        ...
        for i, process in enumerate(processes, start=1):
            ...
            self.process_indices[current_index] = ("process", i - 1)
            current_index += 1
        ...
        self.process_indices[current_index] = ("document_types", 0)
    ...
    def on_process_select(self, event):
        ...
        item_type, item_index = self.process_indices.get(index, (None, None))

        if item_type == "global":
            self.show_global_rules()
        elif item_type == "process":
            processes = self.reglament.get("processes", [])
            if 0 <= item_index < len(processes):
                self.show_process_details(processes[item_index])
        elif item_type == "document_types":
            self.show_document_types()
        else:
            # Сепаратор - ничего не делаем
            pass
